...
///*
		...
		Out << "k_nearst" << "\t" << "n_noises" << "\t" << "n_cls" << "\t"
				<< "real_cls" << "\t" << "itration_to_stop_at" << "\t" << "time_exec" << "\t" << "VMeasure" << "\t" << "FMeasure"
		...
		if (dp_core->reference.size() > 0
			...
			// points with [[ referred_by.size() >= dp_core->refer_to.size())]] have higher weight than others and are considered strong points
				...
				&& dp_core->referred_by.size() >= dp_core->refer_to.size()
				  && stopClustering == false) // for propagation purpose
						{
			//iterNum++;
...
*/
			...
			Out << k_nearst << "\t" << tot_Noises << "\t" <<  itration_to_stop_at << "\t" << time_exec << "\n" ;
			Out.close();

			if (0 >= 1)  // maa phd new //&& n < ds.step_count * 10
					{
			...
			printf("%s %d %d %5.2f %5.2f\n", file_name.c_str(),
					ds.k_nearst, tot_Noises, TP * 100.0,
					TP_BEST * 100.0);
